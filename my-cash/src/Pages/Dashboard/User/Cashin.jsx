...
const Cashin = () => {
    ...
    const HandlCashin=(e)=>{   
        e.preventDefault()
        setError("")
        const cashinamount= parseInt(e.target.amount.value);
        if(cashinamount < 49 ){
        ...
        setError("")
        const CashInInfo={cashinamount,agentnumber,cashinpin,name:user?.name,mobile:user?.mobile,email:user?.email,
          status:user?.status,balance:user?.balance,type:"cashin",cashinstatus:"pending",
        }
        if(CashInInfo.cashinpin){
           ...
           .then(res=>{
            if(res.data.insertedId){
              Swal.fire({
                position: "top-center",
                icon: "success",
                title: "Cashin Request Successfull",
    ...
    return (
        <div>
          <h1 className="text-4xl text-center my-4 uppercase ">Cash In is Free in MyCash</h1>
          <p className="text-center mt-2"> please send a request to cashin from agent in free </p>
